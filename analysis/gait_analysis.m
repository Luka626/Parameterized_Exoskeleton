...
user_waist_circumference = 400;

% GAIT DATA
gait_param = readmatrix('left_right_data_healthy.csv');
...
waist_radius = user_waist_circumference/(2*pi);
...
max_force_angle = gait_param(15,15)*pi/180;
min_force_angle= gait_param(62,15)*pi/180;
...
% PEAK FLEXION
og_y_length = cuff_disp*sin(min_force_angle);
og_x_length = LA_length - cuff_disp*cos(min_force_angle);
og_spring_length = (og_x_length^2 + og_y_length^2)^(1/2);

max_force_param = find_spring_force(spring_coeff, LA_length, cuff_disp, max_force_angle, og_spring_length)
min_force_param = find_spring_force(spring_coeff, LA_length, cuff_disp, min_force_angle, og_spring_length)
...
function [force_param] = find_spring_force(spring_coeff, LA_length, cuff_disp, thigh_LA_angle, og_spring_length)
spring_y_length = cuff_disp*sin(thigh_LA_angle);
spring_x_length = LA_length - cuff_disp*cos(thigh_LA_angle);
