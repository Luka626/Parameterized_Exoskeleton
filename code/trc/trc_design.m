...
while (cost > cost_threshold && num_iterations <= MAX_ITER)
   ...
   %% LOOP
    ...
    if SF_pressure > GOAL_SF || SF_bending > GOAL_SF
        ...
        if shell_outer_radius < shell_inner_radius + 0.001
           shell_outer_radius = shell_inner_radius + 0.001;
        end
    else
        shell_outer_radius = shell_outer_radius + shell_outer_radius*kick;
    end
...
log_to_output(app, sprintf("[trc_design]     SF_fin_cyclical:   %.3f", safety_factors.SF_fin_cyclical));
